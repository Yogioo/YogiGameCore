...

{
    public class CSVTool
    {
        ...
        //读取文件
        public static List<List<string>> ReadStr(string content)
        {
            List<List<string>> result = new List<List<string>>();
            string[] lines = content.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
            for (int i = 0; i < lines.Length; i++)
        ...
        public static List<string> PopulateFromCSV(UnityEngine.TextAsset _csv, int startRowIndex = 1)
        {
            ...
            if (strArrayList.Count > 0)
                return strArrayList;
            UnityEngine.Debug.LogError("There was no data in this CSV: " + _csv.name);
            return null;
        }
