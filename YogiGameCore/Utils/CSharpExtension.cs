...
namespace YogiGameCore.Utils
{
    ...
    public static class IOExtension
    {
        ...
        public static async UniTask<string> ReadTextByWebRequest(this string filePath)
        {
            UnityWebRequest www = UnityWebRequest.Get(new Uri(filePath));
    ...
    public static class StringExtention
    {
        ...
        public static List<int> StringToIntList(this string str, char separator)
        {
            var strArr = str.Split(separator);
            List<int> result = new List<int>();
            for (int i = 0; i < strArr.Length; i++)
            {
                if (strArr[i].IsNullOrEmpty())
                    continue;
                if (int.TryParse(strArr[i],out var intValue))
                {
                    result.Add(intValue);
                }
                else
                {
                    Debug.LogError($"Error Format:{strArr[i]} from {str} in Index:{i}");
                }
            }
            return result;
        }
        public static List<float> StringToFloatList(this string str, char separator)
        {
            ...
            for (int i = 0; i < strArr.Length; i++)
            {
                if (strArr[i].IsNullOrEmpty())
                    continue;
                result.Add(float.Parse(strArr[i]));
            }
            return result;
        }
